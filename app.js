...
// Parallax: deviceorientation + touch fallback
// Extra: particles + light sweep + smooth damping
// Plus: upload BG + subject and save in localStorage
...
function saveScene() {
  ...
  localStorage.setItem("ls3d_scene", JSON.stringify(sceneData));
}
...
subjectFile.addEventListener("change", (e) => {
  ...
  const reader = new FileReader();
  reader.onload = () => {
    saved.subjectDataUrl = reader.result;
...
function setFromOrientation(event) {
  const g = event.gamma ?? 0; // left/right
  const b = event.beta ?? 0;  // front/back

  // normalize and clamp (valori morbidi per evitare "spari" fuori schermo)
  tx = Math.max(-1, Math.min(1, g / 30));
  ty = Math.max(-1, Math.min(1, (b - 20) / 30));
}
...
function frame(now) {
  ...
  const wow = parseInt(wowEl.value || "12", 10);               // intensità
  ...
  // Subject: FIX non mozzata -> movimento più dolce + scala controllata
  subject.style.transform =
    `translate3d(calc(-50% + ${px * wow}px), ${py * (wow * 0.55)}px, 0) scale(${userScale})`;
